...
import { ListeningContent, Difficulty } from '../types';
import { generateListening } from '../services/geminiService';
import { Play, RefreshCw, CheckCircle, XCircle, Headphones } from 'lucide-react';

interface Props {
  difficulty: Difficulty;
}

const PartB: React.FC<Props> = ({ difficulty }) => {
  const [data, setData] = useState<ListeningContent | null>(null);
  ...
  const loadData = async () => {
    ...
    setAnswers({});
    try {
      const content = await generateListening(difficulty);
      setData(content);
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="flex flex-col h-full overflow-y-auto pb-20 px-4 pt-4">
      <div className="bg-white rounded-2xl shadow-sm p-5 mb-4 border border-teal-100">
        <h2 className="text-2xl font-bold text-teal-800 flex items-center gap-2 mb-2">
           <span className="bg-teal-100 p-2 rounded-lg"><Headphones size={24} className="text-teal-600"/></span>
          Część B: Rozumienie ze słuchu
        </h2>
        <p className="text-base text-gray-500">Wysłuchaj nagrania i odpowiedz na pytania.</p>
      </div>

      {loading ? (
        <div className="flex flex-col items-center justify-center h-64">
           <div className="w-12 h-12 border-4 border-teal-500 border-t-transparent rounded-full animate-spin mb-4"></div>
           <p className="text-teal-600 font-medium text-lg">Tworzenie testu...</p>
        </div>
      ) : data ? (
        <div className="space-y-6">
          {/* Audio Control */}
          <div className="bg-teal-900 text-white p-6 rounded-2xl shadow-lg flex flex-col items-center text-center">
             <div className="w-16 h-16 bg-teal-800 rounded-full flex items-center justify-center mb-4">
                <Headphones size={32} />
             </div>
             <h3 className="font-bold text-xl mb-2">Nagranie testowe</h3>
             <p className="text-teal-200 text-base mb-4">Kliknij aby odsłuchać dialog</p>
             
             <button 
              onClick={speak}
              disabled={isPlaying}
              className={`flex items-center gap-2 px-8 py-3 rounded-full font-bold transition transform active:scale-95 text-lg ${isPlaying ? 'bg-gray-500 cursor-not-allowed' : 'bg-white text-teal-900 hover:bg-gray-100'}`}
            >
              {isPlaying ? 'Odtwarzanie...' : 'Odtwórz nagranie'} <Play size={24} fill={isPlaying ? "none" : "currentColor"} />
            </button>
          </div>
